...
class TimeSeries:
    def __init__(self):
        ...
        model_names = ["CNN-LSTM Model", "MQRNN Model"]
        self.model_var = tk.IntVar(value=0)
        ttk.Label(hyperparameter_frame, text="Model Type").grid(column=0, row=3, columnspan=4)
        [tk.Radiobutton(hyperparameter_frame, text=model_names[i], value=i, variable=self.model_var).grid(column=i, row=4) for i in range(2)]
                ...
                if model_choice == 0:
                    ...
                    if i == layers-1:
                        ...
                        for j in range(layers):
                            model.add(cnn_layers[j])
                        for j in range(layers):
                            model.add(lstm_layers[j])
                    else:
